...
function ConvertToClassMainSettingInFile {
    ...
    DoLogs $LogFile02GlobalFQDN " || ConvertToClassMainSettingInFile"
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM1 complete, total count GetDataFromVmguruRaw02 " + $GetDataFromVmguruRaw02.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM2 complete, total count GetDataFromVmguruReady4Sent " + $GetDataFromVmguruReady4Sent.Count)
...
DoLogs $LogFile02GlobalFQDN (" || VMguru done - GetDataFromVmguruReady4Sent " + $GetDataFromVmguruReady4Sent.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Preparing for vmind send  " + $GetDataFromVmindReady4Sent.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Done vmind send - GetDataFromVmindReady4Sent count " + $GetDataFromVmindReady4Sent.Count ) # $GetDataFromVmindRaw02[0]
...
{DoLogs $LogFile02GlobalFQDN (" || Обработка для Dream")}
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM3 complete, total count DreamAnswEditionv23SortedReady4Send " + $DreamAnswEditionv23SortedReady4Send.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM4 complete, total count Body4GetFromEB23RAW02 " + $Body4GetFromEB23RAW02.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM5 complete, total count Body4GetFromCormachogan23RAW02 " + $Body4GetFromCormachogan23RAW02.Count)
...
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM5-1 complete, total count Body4GetFromCHC23RAW03Ready4Send " + $Body4GetFromCHC23RAW03Ready4Send.Count)

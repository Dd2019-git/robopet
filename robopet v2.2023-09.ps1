...
cli - powershell.exe -file "c:\robopet v2.2023-09.ps1" -executionpolicy bypass
...
7.2 Так делать нельзя, потому что время от времени какой-то сайт долго отвечает, и скрипт висит на таймауте, поэтому проще сразу перезаписать файл настроек.
чем потом отправлять по второму разу. 
...
robopet v2.2023-06 - мелкие переделки в логах, исправлена ошибка в функции. 
robopet v2.2023-07 - добавлен "https://williamlam.com" и поправлен баг для перевода в прод. Добавлены сквозные счетчики и таймер. 
Потому что отловил ошибку именно в телеграмме Invoke-WebRequest : {"ok":false,"error_code":429,"description":"Too Many Requests: retry after 811","parameters":{"retry_after":811}}
Можно сначала делать тест вида # update_in_telegram_prod_send_01 $TokenFileProd # Baa Baa и по результатам. 
Ошибка была связана с применением таймаута и к боту тоже, боту добавлено прав.
robopet v2.2023-08 - мелкие изменения в todo
robopet v2.2023-09 мелкие изменения в отчетах про пинг
...
function update_in_telegram_test_send_01 {  #отладочная функция для тестовых прогонов. с жестко забитым "что читаем" !!плохо!!
    ...
    foreach ($BotFindTestId in $TokenFileFQDNContent) {
        if (($BotFindTestId.TokenName -eq  $RunModeUnicID1test)  -and ($BotFindTestId.ItemidCountInRawArray -eq "1")){
...
## жестко забит $RunModeUnicID2prod !!
    ...
    foreach ($BotFindTestId in $F_TokenFileFQDNContent) {
        if (($BotFindTestId.TokenName -eq  $RunModeUnicID2prod)  -and ($BotFindTestId.ItemidCountInRawArray -eq "1")){
            $F_BotToken = $BotFindTestId.BotToken ;             $F_ChatID = $BotFindTestId.ChatID}}
...
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    ...
    $F_Text = "Site update: " + $F_Src +" - " + $F_NewsFromDream.Subject +  " %0A" + "link: " + $F_NewsFromDream.Link
...
function get_updates_from_Williamlam{
    param([string]$F_GlobalWilliamlamURL)
    $F_FullGet01 =  Invoke-WebRequest -Uri $F_GlobalWilliamlamURL
    $F_WilliamlamResult = @()
    $F_FullGet02 = $F_FullGet01.Content
# </ul></div></nav></div><div class="site-inner"><div class="content-sidebar-wrap"><main class="content"><article class="post-179627 post type-post status-publish format-standard category-automation category-vmware-cloud-foundation tag-vmware-cloud-foundation entry" aria-label="Quick Tip - Convert VMware Cloud Foundation (VCF) Deployment Workbook XLSX to JSON" itemscope itemtype="https://schema.org/CreativeWork"><header class="entry-header"><h2 class="entry-title" itemprop="headline"><a class="entry-title-link" rel="bookmark" href="https://williamlam.com/2023/02/quick-tip-convert-vmware-cloud-foundation-vcf-deployment-workbook-xlsx-to-json.html">Quick Tip - Convert VMware Cloud Foundation (VCF) Deployment Workbook XLSX to JSON</a></h2>
    $F_Split01 = 'a class="entry-title-link" rel="bookmark" href="'
    $F_Split02 = '</a></h2>'
    $F_Split03 = '">'
    $F_Split04T1 = 'itemprop="datePublished" datetime="'
    $F_Split05T1 = '">'
    $F_FullPageTextTMP01 = ($F_FullGet02  -Split($F_Split01)) # порезали по числу заголовков новосте

    for ($F_PositionCount=1; $F_PositionCount -lt $F_FullPageTextTMP01.count; $F_PositionCount ++) { 
        $F_SplitMark = 0
        $F_FullPageTextTMP02 = @($F_FullPageTextTMP01[$F_PositionCount] -split $F_Split02) #отбросили хвост
        # $F_FullPageTextTMP02[0] # https://williamlam.com/2023/02/heads-up-24gb-48gb-ddr5-sodimm-mem ory-now-available.html">Heads Up - 24GB &#038; 48GB DDR5 SODIMM memory now available
        $F_FullPageTextTMP03 = @($F_FullPageTextTMP02[0] -split $F_Split03)
        $F_SingleWL = [MainSettingInFile]::new() 
        $F_SingleWL.Link = $F_FullPageTextTMP03[0]
        $F_SingleWL.NumberOrID = $F_FullPageTextTMP03[1]
        $F_SingleWL.Subject = $F_FullPageTextTMP03[1]
        $F_FullPageTextTMP04Time = @($F_FullPageTextTMP02[1] -split $F_Split04T1) # теперь тащим время из текста
        $F_SingleWL.DataFromPost =  $F_FullPageTextTMP04Time[1].Substring(0,19)
        $F_SingleWL.DataSourceOrFrom = $F_GlobalWilliamlamURL
        $F_WilliamlamResult += $F_SingleWL
        Remove-Variable F_SingleWL
        }
    return $F_WilliamlamResult
}
...
$RunMode = "prod"  # other = prod, test, 

$TimeCountBeforeNext = 10
$MaxCountSend = 3
$RunModeUnicID1test = "TestTGbotIDUnicID12964"
$RunModeUnicID2prod = "Prod token1 241D1DFDDA"
$RunModeUnicID = ""



$CurrentScriptVersion = "2.008"
...
If ($RunMode -eq "test") {$TokenFile01 = $TokenFileTest ; 
    $CM = ( $TimeNow.ToString() + " || Load file " + $TokenFileTest + " for current RunMode " + $RunMode); 
    Write-Host $CM ; Out-File $LogFile02GlobalFQDN -Append -NoClobber -InputObject $CM 
    $RunModeUnicID = $RunModeUnicID1test
    }


elseif ($RunMode -eq "prod") {$TokenFile01 = $TokenFileProd ; $CM = ( $TimeNow.ToString() + " current RunMode " + $RunMode); 
    Write-Host $CM ; Out-File $LogFile02GlobalFQDN -Append -NoClobber -InputObject $CM 
    $RunModeUnicID = $RunModeUnicID2prod}
elseif ($RunMode -eq "firstrun") {FirtsRunCreateTOkenFilesTEmplate ; $CM = ( $TimeNow.ToString() + " current RunMode " + $RunMode); 

    Write-Host $CM ; Out-File $LogFile02GlobalFQDN -Append -NoClobber -InputObject $CM ;Exit }
...
$GlobalVmguru = "https://www.vmgu.ru" # добавлен в 2.2023-08, используется только в ping, в процедуру не передается
$TmpURLconverter = $GlobalVmguru.Substring(8,$GlobalVmguru.Length-8)
Write-Host "Testing $TmpURLconverter - timeout 100 sec"
Test-netConnection $TmpURLconverter
Remove-Variable TmpURLconverter

$GetDataFromVmguruRaw01 = get_updates_from_vmguru  # получили список новостей, ВСЕХ. В хз каком формате, не важно. !!Плохо !! сайт жестко забит в функции. 
...
foreach ($BotFindTestIdVMGURU in $TokenFileFQDNContent) {
        if (($BotFindTestIdVMGURU.TokenName -eq  $RunModeUnicID)  -and ($BotFindTestIdVMGURU.ItemidCountInRawArray -eq "1")){
            $F1_BotToken = $BotFindTestIdVMGURU.BotToken ;             $F1_ChatID = $BotFindTestIdVMGURU.ChatID}}

# $F1_BotToken
# $F1_ChatID
$GetDataFromVmguruReady4Sent = ($GetDataFromVmguruReady4Sent | Select-Object -First $MaxCountSend)
...
foreach ($VmguruUpdate23 in $GetDataFromVmguruReady4Sent) { #но это будут все 10, не пойдет. Надо 3. 
    ...
    start-sleep -Seconds $TimeCountBeforeNext
...
$GlobalVmind = "https://vmind.ru"
$TmpURLconverter = $GlobalVmind.Substring(8,$GlobalVmind.Length-8)
Write-Host "Testing $TmpURLconverter - timeout 100 sec"
Test-netConnection $TmpURLconverter
Remove-Variable TmpURLconverter
...
## ИЛи наоборот хорошо, чистка сделана внутри функции, которые могут конечно вернуть и чушь при сбое. и имя сайта жестко забитов в функции. 
...
$GetDataFromVmindReady4Sent = ($GetDataFromVmindReady4Sent | Select-Object -First $MaxCountSend)
...
$DreamAnswEditionv23Sorted = ($DreamAnswEditionv23Sorted | Select-Object -First $MaxCountSend)
...
$DreamAnswEditionv23SortedReady4Send = ($DreamAnswEditionv23SortedReady4Send | Select-Object -First $MaxCountSend)
...
$TmpURLconverter = $GlobalEB.Substring(7,$GlobalEB.Length-7)
Write-Host "Testing $TmpURLconverter - timeout 100 sec"
Test-netConnection $TmpURLconverter
Remove-Variable TmpURLconverter
...
$Body4GetFromEB23RAW03Ready4Send = ($Body4GetFromEB23RAW03Ready4Send | Select-Object -First $MaxCountSend)
...
$TmpURLconverter = $GlobalCormachoganURL.Substring(8,$GlobalCormachoganURL.Length-8)
Write-Host "Testing $TmpURLconverter - timeout 100 sec"
Test-netConnection $TmpURLconverter
Remove-Variable TmpURLconverter
...
$Body4GetFromCHC23RAW03Ready4Send = ($Body4GetFromCHC23RAW03Ready4Send | Select-Object -First $MaxCountSend)
...
## начало блока про https://williamlam.com"
$GlobalWilliamlamURL = "https://williamlam.com"
$TmpURLconverter = $GlobalWilliamlamURL.Substring(8,$GlobalWilliamlamURL.Length-8)
Write-Host "Testing $TmpURLconverter - timeout 100 sec"
Test-netConnection $TmpURLconverter
Remove-Variable TmpURLconverter


DoLogs $LogFile02GlobalFQDN (" || Global start for " + $GlobalWilliamlamURL)
$Body4GetFromWilliamlam23RAW01 = @()
$Body4GetFromWilliamlam23RAW02 = @()
$Body4GetFromWilliamlam23RAW03Ready4Send = @()
$Body4GetFromWilliamlam23RAW01 = get_updates_from_Williamlam $GlobalWilliamlamURL
$Body4GetFromWilliamlam23RAW02 = do_clear2023 $Body4GetFromWilliamlam23RAW01

DoLogs $LogFile02GlobalFQDN (" || Clear mode CM6-1 complete, total count before clear Body4GetFromWilliamlam23RAW01 " + $Body4GetFromWilliamlam23RAW01.Count)
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM6-2 complete, total count after clear Body4GetFromWilliamlam23RAW02 " + $Body4GetFromWilliamlam23RAW02.Count)
# тут бы  логику заменить (ниже) тоже на функцию. 

# и  глобально бы переписать удаление всех временных функций \ переменных после конца задачи, чтобы при копипасте ловить ошибку не определенности функций.

$WilliamlamUpd = "0" #опять перебираем last update in file MainSettingFromFile23 в поисках нашего обновления. 
foreach ($LastUpdTmp in $MainSettingFromFile23) {
    if ($LastUpdTmp.DataSourceOrFrom -eq $GlobalWilliamlamURL) {
        $WilliamlamUpd = $LastUpdTmp.NumberOrID  }}  #Номер получили, при первом запуске все равно будет ноль.
#id выглядит как строка заголовка- например Subject  
# Теперь сравниваем очищенный массив всех новостей $Body4GetFromEB23RAW02 и строку S N="NumberOrID , формируя массив к отправке.
for ($i = 0; $i -lt ($Body4GetFromWilliamlam23RAW02.Count); $i++){
    if ($Body4GetFromWilliamlam23RAW02[$i].NumberOrID -ne $WilliamlamUpd) {
        $Body4GetFromWilliamlam23RAW03Ready4Send += $Body4GetFromWilliamlam23RAW02[$i]}
        else {$i = $Body4GetFromWilliamlam23RAW02.Count} # ну не спортивно, наверное. Но это и не спорт. 
        }
DoLogs $LogFile02GlobalFQDN (" || Clear mode CM6-3 complete, total count Body4GetFromWilliamlam23RAW03Ready4Send " + $Body4GetFromWilliamlam23RAW03Ready4Send.Count)


# и вот это бы в функцию для сохранения настроек. 
if ($WilliamlamUpd -ne "0") {  # фактически 0 будет только при первом запуске и отладке. 
    for ($i = 0; $i -lt ($MainSettingFromFile23.Count); $i++){
    if ($MainSettingFromFile23[$i].DataSourceOrFrom -eq $GlobalWilliamlamURL)   {
        $MainSettingFromFile23[$i] = $Body4GetFromWilliamlam23RAW02[0]  # 
        $Williamlam = "Done"    }}}

Else {$MainSettingFromFile23 += $Body4GetFromWilliamlam23RAW02[0]} 

# тут надо глобально логику поправить, чтобы не каждый раз переписывать текст \ файл конфига. То есть один раз за весь прогон - вполне достаточно.
# Иначе тут уже 5. И убрать это тоже в функцию. 
$MainSettingFromFile23 | Export-Clixml -path $SettingsFileFQDN   

Write-Host "Ready Williamlam send " + $GlobalWilliamlamURL
$Body4GetFromWilliamlam23RAW03Ready4Send = ($Body4GetFromWilliamlam23RAW03Ready4Send | Select-Object -First $MaxCountSend)
foreach ($WLUpdate23 in $Body4GetFromWilliamlam23RAW03Ready4Send) {
    update_in_telegram_from_src_wo_proxy_univ01 $F1_ChatID $F1_BotToken $WLUpdate23 $GlobalWilliamlamURL
    start-sleep -Seconds $TimeCountBeforeNext
}
...
Write-Host "Report 1 - total count GetDataFromVmguruReady4Sent "  $GetDataFromVmguruReady4Sent.Count 
Write-Host "Report 2 - total count GetDataFromVmindReady4Sent "  $GetDataFromVmindReady4Sent.Count  
Write-Host "Report 3 - total count DreamAnswEditionv23SortedReady4Send "  $DreamAnswEditionv23SortedReady4Send.Count
Write-Host "Report 4 - total count Body4GetFromEB23RAW03Ready4Send "  $Body4GetFromEB23RAW03Ready4Send.Count
Write-Host "Report 5 - total count Body4GetFromCHC23RAW03Ready4Send "  $Body4GetFromCHC23RAW03Ready4Send.Count
Write-Host "Report 6 - total count Body4GetFromWilliamlam23RAW03Ready4Send "  $Body4GetFromWilliamlam23RAW03Ready4Send.Count
